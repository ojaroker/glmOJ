...
regression modeling. It wraps `glm`,
[`MASS::glm.nb`](https://rdrr.io/pkg/MASS/man/glm.nb.html), and
[`pscl::zeroinfl`](https://rdrr.io/pkg/pscl/man/zeroinfl.html) to fit,
diagnose, and interpret count regression models, with all output
...
- **`countGLM(formula, data, ziformula, decide = "BIC")`** — Fits all
  four families, selects the best by the criterion in `decide` (`"BIC"`,
...
All coefficients are reported on the **response scale** (exponentiated).
For the count component of any model, exponentiating a coefficient gives
the multiplicative change in the expected count for a one-unit increase
in the predictor, adjusting for simultaneous linear changes in other
predictors. For example, a value of 1.5 means a 50% higher expected
count.
