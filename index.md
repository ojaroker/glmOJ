...
sitting above 1 (the quasi-Poisson signature),
[`quasiPoissonGLM()`](http://oscar.jaroker.com/glmOJ/reference/quasiPoissonGLM.md)
is also fitted and reported alongside — but excluded from the AIC/BIC/
...
- **Variance Inflation Factors (VIFs)** — VIFs are computed for each
  ...
  before interpreting coefficients. Interaction terms and higher power
  terms are grouped to avoid spurious high VIFs from structural
  multicollinearity.
